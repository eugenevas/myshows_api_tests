...
Тесты могут запускаться в режиме CI и на любых развёрнутых стендах (включая продакшен)
Для запуска на стендах:
В .env файле указать BASE_URL, соответствующий адресу стенда, указать данные используемого тестового аккаунта в переменных PROD_TEST_
Запуск в CI:
В .env файле указать BASE_URL=http://localhost (указан по умолчанию). Тестовые данные будут сгенерированы автоматически для каждого прогона фикстурой create_session_data
